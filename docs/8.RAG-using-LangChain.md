...
> Retrievers are **runnables**, meaning they can be composed, chained, and customized as part of RAG (Retrieval-Augmented Generation) pipelines.

![LangChain Retrievers](https://python.langchain.com/assets/images/retriever_concept-1093f15a8f63ddb90bd23decbd249ea5.png)

> Types of retrievers are : 

---

### 🔸 1. Vector Store Retriever  

- The most **common retriever** in LangChain.  
- Retrieves documents from a **vector store** (e.g., FAISS, Chroma, Weaviate, Pinecone) using **semantic similarity**.  

**How it works:**
1. Documents are stored in a vector database.  
2. Each document is converted into a **dense vector** using an **embedding model**.  
3. A query is also embedded into a vector.  
4. The retriever compares the **query vector** with stored vectors.  
5. Returns the **top-k most similar documents**.  

✅ Best for: Semantic search, RAG pipelines, contextual lookups.

---

### 🔸 2. Wikipedia Retriever  

- Uses the **Wikipedia API** to fetch relevant content.  
- Instead of searching in a pre-stored dataset, it queries Wikipedia directly.  

**How it works:**
1. User provides a query (e.g., *"Albert Einstein"*).  
2. Retriever sends the query to **Wikipedia’s API**.  
3. Retrieves the **most relevant article(s)**.  
4. Returns them as LangChain **`Document` objects**.  

✅ Best for: Knowledge retrieval from an external knowledge base without pre-loading documents.

---

### 🔸 3. Maximal Marginal Relevance (MMR) Retriever  

- A retrieval strategy that balances **relevance** and **diversity**.  
- Instead of just returning the top-k most similar documents, it ensures the results are **not redundant**.  

**How it works:**
1. Start with a query vector.  
2. Retrieve an initial pool of candidate documents based on similarity.  
3. Select documents one by one using **MMR scoring**:  
   - High similarity to the query.  
   - Low similarity to already selected documents.  
4. Continue until top-k diverse yet relevant documents are selected.  

✅ Best for: Avoiding repetition, ensuring diverse information coverage.  

---

### 🔸 4. Multi-Query Retriever  

- A retrieval strategy that expands the user query into **multiple semantically related queries**.  
- Useful when a single query may not capture all variations of meaning.  

**How it works:**  
1. Take the original query.  
2. Use an LLM to generate **several reformulations/paraphrases** of the query.  
...
3. Run all expanded queries against the retriever.  
4. Aggregate the results into a final ranked set of documents.  

✅ Best for: Capturing documents that are **relevant but phrased differently**, reducing the risk of missing useful context.  

---

### 🔸 5. Contextual Compression Retriever  

- A retrieval strategy that **filters and compresses documents** to keep only the most relevant parts.  
- Useful when retrieved documents are **too long** or contain **irrelevant sections**.  

**How it works:**  
1. Retrieve initial documents using a base retriever (e.g., VectorStoreRetriever).  
2. Pass those documents through a **compressor** (often an LLM or embedding model).  
3. The compressor trims or rewrites the documents to focus only on content relevant to the query.  
4. Return shorter, high-signal documents to the pipeline.  

✅ Best for: Handling **long documents**, reducing noise, and optimizing context window usage in LLMs.  

---

## 📌 Summary  

| Retriever Type                  | Source of Data                          | Best For |
|---------------------------------|------------------------------------------|----------|
| **VectorStoreRetriever**        | Vector DB (FAISS, Chroma, Pinecone, etc.) | Semantic similarity search |
| **WikipediaRetriever**          | Wikipedia API                           | External factual knowledge |
| **MMR Retriever**               | Any retriever (applied as strategy)      | Balancing relevance + diversity |
| **Multi-Query Retriever**       | Any retriever + LLM for query expansion | Covering varied query formulations |
| **Contextual Compression Retriever** | Any retriever + compressor (LLM/embeddings) | Filtering long docs to keep only relevant parts |

---

🔑 **Tip:** Use **Contextual Compression** with **Multi-Query** or **MMR** to maximize both **recall** (finding diverse documents) and **precision** (keeping only useful chunks) in RAG pipelines.  
